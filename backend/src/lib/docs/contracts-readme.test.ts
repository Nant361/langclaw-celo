...
test("contracts README stays aligned with live public Celo proof references", () => {
  ...
  const isNew = source.includes("decision `#59`") || source.includes("Decision `#59`") || source.includes("decision `59`") || source.includes("decision #59") || source.includes("decision `59`") || source.includes("decision `59`") || source.includes("Registry decision `59`");
