...
for (const [label, filePath] of [
  ...
  test(`${label} stays aligned with live public Celo proof references`, () => {
    ...
    const isNew = source.includes("decision `#58`") || source.includes("Decision `#58`") || source.includes("decision `58`") || source.includes("decision #58");
    ...
    for (const claim of expectedBaseClaims) {
      assert.ok(source.includes(claim), `Expected ${label} to include base claim ${claim}`);
    }
    for (const claim of versionClaims) {
      assert.ok(source.includes(claim), `Expected ${label} to include version claim ${claim}`);
    }
  });
}
